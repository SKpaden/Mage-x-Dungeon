export default class BattleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.logText = this.add.text(50, 650, 'Battle Start!\nTurn: Player Mage', {
      fontSize: '20px', fill: '#fff', wordWrap: { width: 1100, height: 400 }
    });

    // Back to main menu:
    const menuBtn = this.add.rectangle(visualViewport.width/2, visualViewport.height-60, 300, 80, 0x4a7c59).setInteractive();
    menuBtn.on('pointerdown', () => this.scene.start('menu'));  // ← Switch scenes!
    this.add.text(visualViewport.width/2, visualViewport.height-60, 'Main Menu', { fontSize: '24px', fill: '#fff' }).setOrigin(0.5);
